...
def pca_optimize_snr(cube, angle_list, source_xy, fwhm, cube_ref=None,
    ...
    def get_snr(matrix, angle_list, y, x, mode, V, fwhm, ncomp, fmerit,
        ...
        if fmerit == 'max':
            ...
            res = [metrics.snr(frame, (x_, y_), fwhm, plot=False, verbose=False,
            ...
            res = metrics.snr(frame, (x, y), fwhm, plot=False, verbose=False,
                                 full_output=True)
            snrpx = res[-1]
    ...
    if mode == 'fullfr':
        ...
        matrix, annind = prepare_matrix(cube, scaling, None, mode='annular',
                                        inner_radius = 0, outer_radius = ann_radius,
        ...
        if cube_ref is not None:
            ref_lib, _ = prepare_matrix(cube_ref, scaling, mask_center_px,
                                     ...
                                     outer_radius = annulus_width,
                                        # annulus_radius = ann_radius
                                        # annulus_width=annulus_width,
                                     annulus_width = annulus_radius, verbose=False)
        else:
            ref_lib = matrix
